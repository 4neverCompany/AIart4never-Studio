  ...
  const processIdea = useCallback(
    async (
      ...
      const processorDeps: ProcessIdeaDeps = {
        // Web-trending enrichment has been removed. Idea generation will
        // be driven by the agent's own-post analysis in future work; for
        // now there is no live trending context, so this returns '' (the
        // pipeline treats an empty string as "no trending data" and
        // continues without it).
        fetchTrendingContext: async () => '',
