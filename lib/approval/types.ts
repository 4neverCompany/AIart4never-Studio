 ...
 *
 * A SINGLE gate every irreversible action funnels through. Where the existing
 * spend-HIL (`lib/agent-loop/hil.ts`) guards credit burn for generation calls,
 * this module is the generalised, transport-free approval primitive that the
 * publish path, the connector-activation path, and any future irreversible
 * action route through. The contract is proof-based: an approval mints an
 ...
 * approved (denied or timed out). Prototype fix-up mirrors `HilDeniedError`
 * and `McpError` so `instanceof` survives transpilation to ES5-ish targets.
 */
